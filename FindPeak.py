...
def read_file(file_name: str) -> np.ndarray:
    ...
    data = pd.read_csv(file_path, skiprows=10, sep='\t', header=None)
    y_data = data.iloc[5000:, 0].values  # 提取 y 数据
    return y_data

# 使用 ThreadPoolExecutor 并发读取文件
def read_files_concurrently(folder_path: str) -> np.ndarray:
...
class PeakConfig:
    ...
    wavelet_widths: np.ndarray = np.arange(1, 30)
...
if __name__ == "__main__":
    # 读取数据
    # file_path = r"E:\VscodeProject\PeakFind\data\C24071001-0000.txt"
    strat = time.time()
    folder_path = r"E:\VscodeProject\PeakFind\data"
    try:
